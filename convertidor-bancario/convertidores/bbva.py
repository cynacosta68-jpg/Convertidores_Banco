...
# Criterio de persona jurídica: el CUIT empieza con 3.
#   2x (20/23/24/25/26/27) → persona física
#   3x (30/33/34/...)      → persona jurídica → el BBVA la rechaza
DIGITO_JURIDICA = "3"
...
def es_persona_juridica(cuit: str, info: dict = None, cfg: dict = None) -> tuple:
    ...
    Criterio 2 (por defecto): el CUIT empieza con 3.
    ...
    c = (cuit or "").strip()
    if c.startswith(DIGITO_JURIDICA):
        return True, f"CUIT {c} empieza con {DIGITO_JURIDICA} = persona jurídica"
    return False, f"CUIT {c} (persona física)"
