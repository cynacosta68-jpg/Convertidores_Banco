...
def _leer_usuarios():
    """
    Lee la sección [usuarios] de los secrets.

    Devuelve {} si no hay secrets configurados, en vez de romper la app.
    """
    try:
...
def login_required():
    ...
    # Sin secrets configurados nadie podría entrar nunca: avisamos con
    # instrucciones concretas en lugar de dejar una pantalla de login muda.
    if not _leer_usuarios():
        st.error("🔒 Falta configurar los usuarios de esta app.")
        st.markdown(
            "En **share.streamlit.io** → esta app → `⋮` → **Settings** → **Secrets**, "
            "pegá el siguiente bloque y guardá:\n\n"
            "```toml\n"
            "[usuarios]\n"
            'nombre_usuario = "contraseña"\n'
            'otro_usuario = "otra_contraseña"\n'
            "```\n\n"
            "La app se reinicia sola y queda operativa."
        )
        st.stop()
