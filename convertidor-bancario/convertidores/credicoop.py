...
Formato de salida (13 campos separados por ';'):
  CBU(22) ; Monto(15 = 12 enteros + ',' + 2 dec) ; TitularDesc(100) ; TitularCui(11) ;
...
# El campo de monto del modelo Credicoop son 12 dígitos enteros + ',' + 2
# decimales = 15 caracteres. Capacidad: hasta $999.999.999.999,99.
ENTEROS_MONTO = 12
MONTO_MAXIMO  = 10 ** ENTEROS_MONTO - 1        # 999.999.999.999 pesos


def formato_monto(monto) -> str:
    """
    Convierte el monto en centavos a '000000032052,74'.

    El campo admite hasta $999.999.999.999,99, así que cualquier importe real
    de la liquidación entra completo. Ejemplos:
        $223.099,69      -> '000000223099,69'
        $120.000.000,00  -> '000120000000,00'
        $104.074.684,23  -> '000104074684,23'

    Acepta un int (lo que devuelve otros_bancos) o el string de dígitos del
    archivo de origen, por compatibilidad.

    Lanza ValueError si el importe no entra en el campo: es preferible frenar
    el proceso antes que emitir un registro que el banco rechazaría.
    """
    valor = monto if isinstance(monto, int) else int(str(monto).strip() or 0)
    entero = valor // 100
    decimal = valor % 100
    if entero > MONTO_MAXIMO:
        raise ValueError(
            f"El importe ${entero:,}.{decimal:02d} supera la capacidad del campo "
            f"de monto de Credicoop (máximo $999.999.999.999,99)."
        )
    return f"{entero:0{ENTEROS_MONTO}d},{decimal:02d}"
...
def convertir(input_bytes: bytes, maestro_bytes: bytes, config: dict = None) -> dict:
    ...
    # Se excluyen los bancos que tienen su propio circuito (285 MACRO,
    # 017 BBVA). El parser detecta la codificación real y reacomoda las
    # líneas corridas antes de filtrar.
    todos = otros_bancos.parsear(input_bytes)
    excluidos_cod = (otros_bancos.COD_MACRO, otros_bancos.COD_BBVA)
    regs = [r for r in todos["registros"] if r["banco"] not in excluidos_cod]
    descartadas = len(todos["registros"]) - len(regs)
    ...
    for r in regs:
        ...
        try:
            monto_fmt = formato_monto(r["importe"])
        except ValueError as e:
            fuera_de_rango.append({"cbu": cbu, "nombre": r["nombre"], "detalle": str(e)})
            continue

        linea = construir_linea(
            cbu=cbu, monto_fmt=monto_fmt, nombre_100=r["nombre"],
            cuit_11=r["cuit"], concepto_3=r["concepto"],
            desc_12=r["desc_concepto"],
    ...
    if not salida:
        return {
            ...
            "encoding":  todos["encoding"],
            "anomalias": todos["anomalias"],
            "corridas":  0,
            "ilegibles": len(todos["descartados"]),
    ...
    return {
        ...
        "encoding":     todos["encoding"],
        "anomalias":    [a for a in todos["anomalias"]
                         if not a["cbu"].startswith(excluidos_cod)],
        "corridas":     sum(1 for r in regs if r["corrido"]),
        "ilegibles":    len(todos["descartados"]),
        "fuera_de_rango": fuera_de_rango,
        "ampliados":    sum(1 for r in regs if r["campo_ampliado"]),
    }
