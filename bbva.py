...
- Extrae filas que comienzan con "017" del archivo de input
...
def parsear_fila_input(linea: str) -> dict:
    return {
        "cbu":       linea[0:22],
        "importe":   int(linea[22:32]),
        "nombre":    linea[32:132].strip(),
        "tipo_doc":  linea[197:198],
        "cuil":      linea[198:209],
    }
...
def convertir(input_bytes: bytes, maestro_bytes: bytes,
    ...
    # Leer y filtrar filas "017"
    texto = input_bytes.decode("latin-1")
    filas = []
    for linea in texto.splitlines():
        if linea.startswith("017"):
            filas.append(parsear_fila_input(linea))

    if not filas:
        return {
            "contenido": "",
            "beneficiarios": 0,
            "error": "No se encontraron filas '017' en el archivo de entrada."
        }
